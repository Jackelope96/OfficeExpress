...
Public Class Settings
	...
	Public Shared ReadOnly Property CurrentUserID As Integer
		Get
			'Singular.Misc.IsNull(CType(Csla.ApplicationContext.User, Singular.Security.Principal).GetIdentity.UserID, 0) = 0) AndAlso
			...
			'ElseIf Singular.Misc.IsNull(CType(Csla.ApplicationContext.User, Singular.Security.Principal).GetIdentity.UserID, 0) <> 0 Then
			'  Return CType(Csla.ApplicationContext.User, Singular.Security.Principal).GetIdentity.UserID
			'Else
...
#Else
	...
	Public Overloads Shared ReadOnly Property ConnectionString As String
		Get
			' Return "Data Source=WEBBERBRE\SQL08NotR2;Initial Catalog=MCoSQL_Dev;Integrated Security=SSPI;"
			'Return "Data Source=192.168.50.46;Initial Catalog=PenbevSAM;uid=sssadmin1;password=s1ngul@r"
	...
	Protected Shared Function GetConnectionStringFromSettings(ByVal SectionName As String) As String
		...
		'Return "Data Source=singularfs1\sql2008;Integrated Security=SSPI;Initial Catalog=MCoSQL_Dev"
...
#Region " Get and Set Settings "
	...
	Public Shared Function HasSetting(ByVal SectionName As String, ByVal PropertyName As String) As Boolean
		...
		'  If Not IsNothing(MiscData) AndAlso MiscData.Rows.Count > 0 AndAlso MiscData.Columns.Contains(PropertyName) Then
		'    Return True
		'  Else
		'    Return False
		'  End If
	...
	Public Shared Function GetSetting(ByVal SectionName As String, ByVal PropertyName As String, Optional ByVal DefaultValue As Object = "") As Object
		...
		Try
			...
			'If SectionName = "Misc" Then
			'  Return GetMiscSetting(PropertyName)
			'  Exit Function
			'End If
			...
			If mData Is Nothing Then
				...
				If IO.File.Exists(primaryPath) Then
					...
					'Dim FileText As String = IO.File.ReadAllText(primaryPath)
					'If FileText.Contains("<NewDataSet>" & vbCrLf & _
					'                            "  <NewDataSet>") Then
					'  FileText = FileText.Replace("<NewDataSet>" & vbCrLf & _
					'                            "  <NewDataSet>", "<NewDataSet>").Replace("  </NewDataSet>" & vbCrLf & _
					'                                                                      "  <LoadTransactions />" & vbCrLf & _
					'                                                                      "</NewDataSet>", "</NewDataSet>")
					'  IO.File.WriteAllText(primaryPath, FileText)
					'End If
...
#Region " Connection String "
	...
	Public Shared Sub SetConnectionStringFromSettings(ByVal SectionName As String)
		...
		'If (mConnectionString = "" Or mConnectionString = "Data Source=;Integrated Security=SSPI;Initial Catalog=" Or mLastConnectionSectionName <> SectionName) And Csla.ApplicationContext.DataPortalProxy.Contains("Local") Then
...
#Region " Test Connection "
	...
	Public Shared Function TestConnection(ByVal Server As String, ByVal Database As String, ByVal Integrated As Boolean, ByVal UserName As String, ByVal Password As String, ByRef Exception As Exception,
		...
		'  Try
		'    Dim ts As TestSettings = Nothing
		'    Try
		'      ts = TestSettings.TestRemotingConnectionSettings
		'    Catch ex As Exception
		'      Dim WebException As Exception = Nothing
		'      If Singular.Misc.Debug.ExceptionContainsExceptionType(ex, GetType(System.Net.WebException), WebException) Then
		'        If WebException.Message.Contains("417") Then
		'          System.Net.ServicePointManager.Expect100Continue = False
		'          ts = TestSettings.TestRemotingConnectionSettings
		'        ElseIf WebException.Message.Contains("407") Then
		'          Return ConnectionTestResults.ProxyAuthenticationRequired407
		'        End If
		'      Else
		'        Throw ex
		'      End If
		...
		'      If Not String.IsNullOrEmpty(ts.CustomConnectionTestResults) Then
		'        Exception = New Exception(ts.CustomConnectionTestResults)
		'        Return ConnectionTestResults.CantFindServer
		'      End If
		'      Return ts.ConnectionTestResults
		'    Else
		'      Return ConnectionTestResults.CantFindDatabase
		'    End If
		...
		Try
			...
			Using cmd As New SqlClient.SqlCommand("SELECT TOP 1 * FROM dbo.sysdatabases WHERE name = @DBName")
				...
				End Using
				'Dim cproc As New Singular.CommandProc(cmd, CommandProc.FetchTypes.DataObject, CommandType.Text)
				'cproc.Execute()
				'If cproc.DataObject Is Nothing Then
				'  Exception = New Exception("Cannot Find Database")
				'  Return ConnectionTestResults.CantFindDatabase
				'End If
