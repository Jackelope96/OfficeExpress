...
#If Silverlight = False Then
    ...
    Public Sub PrepareToSave()
      ...
      For Each ss As SystemSetting In Me
        Dim ssLocal = ss
        ss.Settings.GetType.ForEachBrowsableProperty(Nothing, Sub(pi)
    ...
    Friend Sub PopulateSettings(FilterName As String)
      ...
      For Each t As Type In SystemSettings.SettingsTypeList

        Dim ss As ISettingsSection = Activator.CreateInstance(t)
      ...
      For Each ss In Me
        If ss.Settings Is Nothing Then
          Throw New Exception(ss.SystemSetting & " is in the System Settings table, but does not have a settings class registered.")
        End If
      Next
...
#Region "  .Net Data Access  "
    ...
    Public Shared Function GetSystemSettingList(SettingName As String) As SystemSettingList
      Return DataPortal.Fetch(Of SystemSettingList)(New Criteria() With {.SettingName = SettingName})
    End Function

    Public Shared Function GetSystemSettingList(SettingType As Type) As SystemSettingList

      Dim ss As ISettingsSection = Activator.CreateInstance(SettingType)
      Dim ssl = DataPortal.Fetch(Of SystemSettingList)(New Criteria() With {.SettingName = ss.Name, .SettingsSection = ss})
    ...
    Private Sub Fetch(sdr As SafeDataReader)
      ...
      For Each ssType In SystemSettings.SettingsTypeList
        Dim pis = ssType.GetProperties(System.Reflection.BindingFlags.Public + System.Reflection.BindingFlags.Instance)
        ...
        If encryptedProperties.Count > 0 Then
          ...
          Dim tempSettings As ISettingsSection = Activator.CreateInstance(ssType)
          Dim ss = Me.Find(tempSettings.Name)
