    ...
    Protected Friend Overrides Sub Setup()
      ...
      If PropertyInfo IsNot Nothing Then
        ...
        If mTFA IsNot Nothing AndAlso mTFA.MultiLine Then
          AddBinding(KnockoutBindingString.html, GetForJS() & "().replace(/\r\n?|\n/g, '<br />')")
        ElseIf mDDA IsNot Nothing Then
          ...
          If mDDA.LookupMember <> "" Then
            ...
            AddBinding(KnockoutBindingString.html, mDDA.LookupMember)
          Else
...
#End Region
  ...
  Public Class Image(Of ObjectType)
    ...
    Public Sub HashSrc(PropertyOrPath As String, Optional MissingPath As String = "")
      ...
      If MissingPath.StartsWith("~") Then
        MissingPath = VirtualPathUtility.ToAbsolute(MissingPath)
      End If

      AddBinding(Singular.Web.KnockoutBindingString.src,
                                 "(" & PropertyOrPath & " ? '" & VirtualPathUtility.ToAbsolute("~/Library/FileDownloader.ashx") & "?DocumentHash=' + " & PropertyOrPath & " : '" & MissingPath & "')")
      ...
      With Writer
        ...
        Else
          ...
          If Src.Contains("~") Then
            Attributes.Add("src", VirtualPathUtility.ToAbsolute(Src))
  ...
  Public Class AjaxControlLoader
    ...
    Private mContainer As Singular.Web.ContainerType

    Public Shared KeyLookup As New Hashtable
    Public Shared RenderedControls As New Hashtable

    Public Sub New(ControlType As Type, Optional Container As Singular.Web.ContainerType = ContainerType.Div, Optional ID As String = "")
      ...
      CalculateHash(Key,
                    Function()
                      Return Activator.CreateInstance(ControlType, True)
                    End Function)

    End Sub

    Private Sub CalculateHash(Key As String, GetControl As Func(Of HelperBase))

      Dim HashCode As String

      'Get the hash code for this control type.
      If KeyLookup.ContainsKey(Key) Then
        HashCode = KeyLookup(Key)
      Else
        Dim Control As HelperBase = GetControl()
        If Not Control.IsSetup Then
          Control.Setup()
        End If
        Dim ControlHtml As String = Control.GetHTMLString
        HashCode = Singular.Encryption.GetStringHash(ControlHtml, Encryption.HashType.MD5, False, Encryption.ASCIIEncoding.ASCII)
        SyncLock KeyLookup
          KeyLookup(Key) = HashCode
          RenderedControls(HashCode) = ControlHtml
        End SyncLock
      End If

      Attributes.Add("data-AsyncContent", HashCode)

    End Sub

    Friend Shared Function GetHTML(Hash As String) As String
  ...
  Public Class DateRange(Of ObjectType)
    ...
    Private Function GetJS(Func As System.Linq.Expressions.Expression(Of System.Func(Of ObjectType, Object))) As String
      Dim Output = Singular.Linq.JavascriptExpressionParser(Of ObjectType).GetCachedOutput(Func, Linq.JavascriptExpressionParser(Of ObjectType).OutputModeType.KnockoutBinding, True)
