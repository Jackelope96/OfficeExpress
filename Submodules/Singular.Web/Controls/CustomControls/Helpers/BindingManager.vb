...
#Region " Add Bindings "
    ...
    Public Sub AddDialogBinding(VisibleConditionJS As String, Title As String, BeforeCloseFunctionName As String,
      ...
      Add(KnockoutBindingString.dialog, "{ Show: " & VisibleConditionJS &
          ", Title: " & Title &
    ...
    Public Sub AddDateBinding(ListProperty As System.Linq.Expressions.Expression(Of System.Func(Of ObjectType, Object)))
      ...
      Dim expr = Singular.Linq.JavascriptExpressionParser(Of ObjectType).GetCachedOutput(ListProperty,
                                                                                        Linq.JavascriptExpressionParser(Of ObjectType).OutputModeType.KnockoutBinding, True,
                                                                                        Linq.JavascriptExpressionParser(Of ObjectType).EqualsActionType.Compare)
      ...
      If mParent.IsBindingAllowed(BindingType) Then
        ...
        Select Case BindingType
          ...
          Case KnockoutBindingString.with
            Me(KnockoutBindingString.template) = "{ 'if': " & Binding & ", data: " & Binding & ", afterRender: Singular.AfterTemplateRender }"
          Case KnockoutBindingString.if
            Me(KnockoutBindingString.template) = "{ 'if': " & Binding & ", afterRender: Singular.AfterTemplateRender }"
...
#Region " Methods "
    ...
    Public Shared Function CreateBindingExpression(ByVal BindingType As KnockoutBindingString,
      ...
      Try
        Return Singular.Linq.JavascriptExpressionParser(Of ObjectType).GetCachedOutput(le,
                                                                                        Linq.JavascriptExpressionParser(Of ObjectType).OutputModeType.KnockoutBinding,
                                                                                        True,
                                                                                        If(BindingType = KnockoutBindingString.click, Linq.JavascriptExpressionParser(Of ObjectType).EqualsActionType.SetValue, Linq.JavascriptExpressionParser(Of ObjectType).EqualsActionType.Compare),
                                                                                        If(JSonBindings, Linq.JavascriptExpressionParser(Of ObjectType).PropertyAccessType.PlainJSon, Linq.JavascriptExpressionParser(Of ObjectType).PropertyAccessType.AutoDetect)) _
                                                                                      .JavascriptString
...
#Region " Number / Date value bindings "
