    ...
    Protected Overrides Sub NoDataSourceSection(Helpers As HelperAccessors(Of SGridInfo))
      ...
      With Helpers.Div
        ...
        With .Helpers.HTMLTag("p")
          .Helpers.LinkFor(, , VirtualPathUtility.ToAbsolute("~/"), "Click here")
  ...
  Public Class SGridServer
    ...
    Public Class Band
      ...
      Private Function CreateSummaryColumn(Col As Object, OrigColumn As DataColumn) As SummaryColumn
        ...
        If OrigColumn Is Nothing AndAlso Col.SummaryType <> SGridServer.SummaryType.Count Then
          'needs to be fixed
          Return Nothing
        Else
          Dim Key = If(Col.SummaryType = SGridServer.SummaryType.Count, "Count", CStr(Col.Key))
          If Col.CTKey IsNot Nothing Then
          ...
          Dim SCol As New SummaryColumn(Table.Columns.Add(Key, If(Col.SummaryType = SGridServer.SummaryType.Count Or Col.SummaryType = SGridServer.SummaryType.DistinctCount, GetType(Integer), SummaryColumnDataType)))
          ...
          If Col.FKey IsNot Nothing Then
            ...
            SCol.Filter = New ColumnFilter()
            SCol.Filter.Type = Col.FType
            SCol.Filter.FilterValue = Col.FValue
      ...
      Friend Function Match(Value As Object) As Boolean
        ...
        Select Case Type
          ...
          Case FilterType.StartsWith
            Return CStr(Value).StartsWith(FilterValue)
          Case FilterType.Contains
            Return CStr(Value).Contains(FilterValue)
          Case FilterType.EndsWith
            Return CStr(Value).EndsWith(FilterValue)
          Case FilterType.DoesntStartWith
            Return Not CStr(Value).StartsWith(FilterValue)
          Case FilterType.DoesntContain
            Return Not CStr(Value).Contains(FilterValue)
          Case FilterType.DoesntEndWith
            Return Not CStr(Value).EndsWith(FilterValue)
      ...
      Friend Sub LoadLayout(ColLayout As Object)
        ...
        If ColLayout.Filters IsNot Nothing Then
          For Each Filter As Object In ColLayout.Filters
            Dim f As New ColumnFilter
            f.Type = Filter.Type
            f.FilterValue = Filter.Value
            If Me.DataColumn.DataType Is GetType(Date) AndAlso TypeOf (Filter.Value) Is Long Then
              f.FilterValue = New Date(1970, 1, 1).AddMilliseconds(f.FilterValue).Add(TimeZone.CurrentTimeZone.GetUtcOffset(Now))
            End If
            Filters.Add(f)
          Next
