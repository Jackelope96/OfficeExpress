    ...
    Protected Overrides Sub RegisterGraphResources(Page As PageBase)
      ...
      For Each s In mChartSettings.mSeriesList
        If s.HighName IsNot Nothing Then
          Page.AddLateResource(ChartJSFiles.JQOHLCRenderer.ScriptTag)
          Exit For
        End If
      Next

      If mChartSettings.XAxis.AxisRenderType = Charting.AxisRenderer.DateRenderer Then
        Page.AddLateResource(ChartJSFiles.JQDateAxisRenderer.ScriptTag)
      End If

      If mChartSettings.XAxis.AxisRenderType = Charting.AxisRenderer.CategoryRenderer Then
        Page.AddLateResource(ChartJSFiles.JQPlotcategoryAxisRenderer.ScriptTag)
      End If

      If mChartSettings.XAxis.TickRenderer = AxisLabelRenderer.CanvasAxisTickRenderer Then
        Page.AddLateResource(ChartJSFiles.JQCanvasAxisTickRenderer.ScriptTag)
        Page.AddLateResource(ChartJSFiles.JQCanvasTextRenderer.ScriptTag)
      End If

      CType(Page, Singular.Web.PageBase).AddLateResource(ChartJSFiles.JQPlothighlighter.ScriptTag)
      CType(Page, Singular.Web.PageBase).AddLateResource(ChartJSFiles.JQDateAxisRenderer.ScriptTag)
