...
Singular.Combobox = function (TextInput, KOBindings) {
  ...
  self.GetSelectedInfo = function (external) {
    ...
    if ((SelectedValue != null && SelectedValue !== '') || (Bindings.LookupMember)) {
      ...
      if (Bindings.Source) {
        Source = ko.utils.unwrapObservable(Bindings.Source);
      } else {
  ...
  self.SetDisplayText = function (external) {
    ...
    if (SelectedInfo) {
      ...
      if (Bindings.AllowNotInList && TextInput.value && TextInput.value != Caption) {
  ...
  function BuildDropDownItems() {
    ...
    if ((OldSelectedID == null || OldSelectedID == '') && FilteredItems.length == 1 && TextInput.value != '') {
      SelectedIndex = 0;
