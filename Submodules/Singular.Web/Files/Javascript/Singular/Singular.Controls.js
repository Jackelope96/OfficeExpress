...
Singular.ShowFindDialog = function (Data, Criteria, Key, AutoPopulate, PreFind, RowClick, MultiSelect, PreSearch) {
  ...
  if (PreFind) {
    var Args = { Control: null /*needed?*/, Data: Data, Object: Data, AutoPopulate: AutoPopulate, FindDialog: JDialog, Criteria: Criteria() };
    PreFind(Args);
...
Singular.UploadFile = function (File, Doc, AllowedExtensions, CallBack, OtherQSParams, Options) {
  ...
  var AjaxSender,
      UploadPath,
      FileName = File ? File.name : fu.value;
    ...
    if (Doc) {
      if (Result.Success) {
        ...
        if (Result.DocumentID) {
          ...
          if (Parent) Parent.IsSelfDirty(true); //usually the document is saved as part of the parent object, so just mark it as dirty.
        }
  ...
  if (AjaxSender) {
    ...
    form.method = "POST";
    form.action = UploadPath + '&FileName=' + fu.value;
    form.id = "uploadForm";
...
Singular.ShowMessage = function (Title, Message, IconType, AfterCloseHandler) {
  ...
  Singular.ShowDialog(div, { title: Title, modal: true, width: Math.min(400, window.innerWidth - 20/*scrollbar*/) + 'px', buttons: [{ text: "Ok", click: function () { div.DlgClose(); AfterCloseHandler() } }], destroyAfter: true });
};
  ...
  $(Table).find('th').each(function () {
    ...
    if (Header.attr('data-Property')) {
      ...
      Header.click(function (event) {
        ...
        //Set the sort icon
        Singular.FormatSortableHeader(Table, this, SortOptions.SortAsc);

        //Re-format the alt rows.
        Singular.FormatTableBody($(Table).find('tbody')[0]);
...
Singular.FormatSortableHeader = function (Table, Header, Asc) {
  ...
  if (Singular.UIStyle && Singular.UIStyle == 2) {
    ...
    if (PrevHead) {
      $(PrevHead).removeClass('column-sorted'); if (PrevHead._SortImg) {
    ...
    if (!Header._SortImg) {
      ...
      if (Asc) {
        img.className = "fa fa-chevron-up sort-icon";
      } else {
        img.className = "fa fa-chevron-down sort-icon";
      }
      Header.appendChild(img)
      Header._SortImg = img
    } else {
      if (Asc) {
        Header._SortImg.className = "fa fa-chevron-up sort-icon";
      } else {
        Header._SortImg.className = "fa fa-chevron-down sort-icon";
      }
    }
...
Singular.ScrollableTable = function (Table) {
  ...
  jTable.find('thead tr, tfoot tr').each(function () {
  ...
  tbody.css({
    ...
    display: 'block', 'overflow-y': MaxHeight ? 'scroll' : ''
  });
  ...
  var ScrollWidth = tbody[0].offsetWidth - tbody[0].clientWidth;
  ...
  jTable.find('thead tr').each(function () {
    ...
    if (BodyRows.length > RowIndex) {
      var BodyCells = $(BodyRows[RowIndex]).children('td');
      if (HeaderCells.length == BodyCells.length) {
        for (var i = 0; i < HeaderCells.length; i++) {
          var tdWidth = $(BodyCells[i]).outerWidth();
...
Singular.CreateLoadingBar = function () {
  ...
  img.src = Singular.RootPath + '/Singular/Images/Loading.gif';
  img.alt = 'Loading...'
...
Singular.GetAuthImage = function (AuthValue) {
  ...
  if (AuthValue == null)
    ...
    return Singular.RootPath + '/Singular/Images/' + (AuthValue == 0 ? 'IconEdit' : (AuthValue == 1 ? 'IconAuth' : 'IconError')) + '.png';
}
