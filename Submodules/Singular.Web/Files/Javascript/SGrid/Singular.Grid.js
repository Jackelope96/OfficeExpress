...
Singular.SGrid = function (Canvas, SetupArgs) {
          ...
          if (col.TotalSum) {
            ...
            for (var j = 0; j < ChildRows.length; j++) {
              if (!ChildRows[j]._SGrid.IsFiltered()) {
                Sum += ChildRows[j][col.Key];
              }
            }
            Totals[this.Band.Key][col.Key] = Sum;
          }
        }
      }
  ...
  function Column(band, Key) {
    ...
    this._RightPadding = function () {
      return (this.Band.HeaderStyle.PaddingH) + (pvt.ShowFilter ? COL_FILTER_WIDTH : 0) + (pvt.ShowSort ? COL_SORT_WIDTH : 0);
    }
    ...
    this.CellTextSized = function (row) {
      ...
      if (row) {
        ...
        MaxWidth -= (this.Band.HeaderStyle.PaddingH + this._RightPadding());
      }
      if (pvt.BracketNegatives) {
        //Take bracket width into account.
        if (!row || row[thisCol.Key] >= 0) {
    ...
    this.CreateGroupByBand = function (ChildBand) {
      ...
      for (var i = 0; i < ColList.length; i++) {
        ...
        if (col.Visible && col != this && col.Type == 'n' && !col._.IsID) {
          Column.CreateSummaryColumn(Singular.SGrid.SummaryTypes.Sum, newBand, col);
        }
      }
  ...
  var EditorValueChanged = function (e) {
    ...
    var Args = { Row: cmi.Row, Column: cmi.Column, Value: e.target.value }
    
    if (cmi.Column.Type == 'n') Args.Value = parseFloat(Args.Value);
    if (cmi.Column.Type == 'd') Args.Value = Date.parse(Args.Value);
    if (cmi.Column.Type == 'b') Args.Value = Args.Value == 'true';
    cmi.Row[cmi.Column.Key] = Args.Value;

    if (_Opt.AfterCellEdit) {
      _Opt.AfterCellEdit(Args);
    }
  ...
  self.ResetRow = function (RowInfo) {
    ...
    if (RowInfo.Band.ParentBand) RowInfo.Band.ParentBand.RefilterGrouping(false, RowIndexes[RowInfo.RowIndex].ParentRi);
  ...
  var DrawGrid = function (Grid) {
    ...
    for (var j = 0; j < Band.Columns.length && ColLeft < ScreenRight; j++) {
      ...
      if (Col.Visible) {
        ...
        if (!Col._.OffScreen) {
          ...
          //Draw header text 
          Context.fillStyle = self.DefaultStyles.HeaderStyle.ForeColor;
          ...
          if (Col._.ShowSort) {
            //sort indicator.
            var Offset = (Col._.ShowFilter ? COL_FILTER_WIDTH : 0) + COL_SORT_WIDTH;

            Context.font = '12px FontAwesome';
            if (Col.SortDirection == 0 || Hovering) {
            ...
            if (Col.SortDirection == 1 || Hovering) {
    ...
    if (_DrawIndex > 0 && RowIndexes[_DrawIndex - 1].LastChild) {
      ...
      //if (Band.Level > 0) {
        ...
        Context.lineTo(x + RowSelectorWidth - 0.5, _YPos + Band.HeaderStyle.Height);
        ...
        for (var j = 0; j < Band.Columns.length && (!Band.Columns[j].Visible || Band.Columns[j]._.Left < ScreenRight) ; j++) {
          ...
          if (j == Band.FrozenColumn + 1 && xOffset > 0) {
            ...
            Context.save();
            Context.beginPath();
            Context.rect(ColLeft, _YPos - 1, HeaderWidth, _YPos + Band.HeaderStyle.Height);
            Context.clip();
            IsClipping = true;
            Context.beginPath();
          ...
          if (Col.Visible) {
            ...
            if (!Col._.OffScreen) {
              ...
              if (j < Band.Columns.length) {
                ...
                Context.lineTo(ColLeft + Col.Width - 0.5, _YPos + Band.HeaderStyle.Height);
              }
            }
            ColLeft += Col.Width;
          }
...
Singular.SGrid.GroupBy = function (Band) {
  ...
  if (Band.GroupByInfo) {
    ...
    for (var i = 0 ; i < Band._SummaryColumns.length; i++) {
      ...
      this.SummaryColumns.push(new Singular.SGrid.SummaryColumn(self, Band.ChildBands[0], col));
    }
  } else {
    //Normal, non group by band.
    for (var i = 0; i < Band.Columns.length; i++) {
      this.SummaryColumns.push(new Singular.SGrid.SummaryColumn(self, Band, Band.Columns[i]));
    }
  }
}
...
Singular.SGrid.SummaryColumn = function (Parent, SummaryBand, GridColumn) {
  var self = this;
  ...
  self.Total = ko.observable(GridColumn ? GridColumn.TotalSum : false);
  self.Total.subscribe(function(value){
    GridColumn.TotalSum = value;
