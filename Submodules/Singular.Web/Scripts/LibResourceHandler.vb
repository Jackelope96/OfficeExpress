Namespace Scripts
  ...
  Public Class LibResourceHandler
    ...
    Private Shared _PhysicalFilesPath As String
      ...
      If Not String.IsNullOrEmpty(FileName) Then
        ...
        If Type = "d" Then
          ...
          If _HasPhysicalFiles Is Nothing Then
            _PhysicalFilesPath = IO.Path.Combine(context.Server.MapPath("~"), "..\..\Singular.Web\Files")
            _HasPhysicalFiles = IO.Directory.Exists(_PhysicalFilesPath)
          End If

          If _HasPhysicalFiles Then
            context.Response.WriteFile(IO.Path.Combine(_PhysicalFilesPath, RelativePath, FileName))
            Exit Sub
          End If
    ...
    Friend Shared Function GetURL(FileName As String, Path As String, Type As String, VersionNo As String, Optional AddLevels As Boolean = False) As String
      ...
      Return String.Format("{0}?type={1}&v={2}&path={3}&file={4}", VirtualPathUtility.ToAbsolute(HandlerPath & FileName.Replace(".", "_")),
                                                                                                 Type, VersionNo, Path, FileName)
    End Function

    Friend Shared Function GetEmbeddedContent(FileName As String) As Byte()
