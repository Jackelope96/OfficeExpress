...
Namespace Scripts
  ...
  Public Class ScriptFile
    ...
    Private ReadOnly Property HasVirtualFolder As Boolean
      Get
        If _HasVirtualFolder Is Nothing Then
          Try
            Dim PhysicalPath = System.Web.HttpContext.Current.Server.MapPath("~/Singular")
      ...
      Get
        If String.IsNullOrEmpty(_ScriptPath) Then
          ...
          If UseDebugScripts OrElse DebugOnly Then
            _ScriptPath = VirtualPathUtility.ToAbsolute(Path) & "/" & DebugScriptName
            IsDebug = True
            Type = "d"
          Else
            If CDNPath = "" OrElse Not Settings.UseCDN Then
              _ScriptPath = VirtualPathUtility.ToAbsolute(Path) & "/" & ReleaseScriptName
            Else
              _ScriptPath = CDNPath
            End If
          ...
          Dim LibPath = VirtualPathUtility.ToAbsolute("~/" & LibResourceHandler.HandlerURL)
          ...
          If _ScriptPath.StartsWith(LibPath) Then
            ...
            If HasVirtualFolder AndAlso IsDebug Then
              ...
              _ScriptPath = _ScriptPath.Replace(LibPath, VirtualPathUtility.ToAbsolute("~/Singular"))
            Else
    ...
    Public Shared Function GetScriptTag(FilePath As String) As String
      If FilePath.StartsWith("~") Then
        FilePath = VirtualPathUtility.ToAbsolute(FilePath)
      End If
      Return vbCrLf & "<script src=""" & FilePath & """ type=""text/javascript"" ></script>"
    End Function
    ...
    Friend Function ReadContents(Path As String) As Byte()
      ...
      If _FileBytes Is Nothing Then
        ...
        Else
          _FileBytes = IO.File.ReadAllBytes(IO.Path.Combine(System.Web.HttpContext.Current.Server.MapPath(Path), DebugScriptName))
        End If
      End If
