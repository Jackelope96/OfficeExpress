...
Public MustInherit Class PageBase
  Inherits Page
  Implements Data.IViewModelPage
...
#Region " Local Session "
  ...
  Public ReadOnly Property PageGuid As Guid Implements Data.IViewModelPage.PageGuid
    Get
      Return mPageGuid
    End Get
  End Property
...
#End Region
  ...
  Public Sub AddLateResource(IncludeString As String)
  ...
  Public Sub AddLateScript(Source As String)
    AddLateResource("<script type=""text/javascript"" src=""" & VirtualPathUtility.ToAbsolute(Source) & """></script>")
  End Sub
...
#End Region
  ...
  Protected Overrides Sub OnPreInit(e As System.EventArgs)
    ...
    If Request.QueryString("SCmd") = "Logout" AndAlso Singular.Security.CurrentIdentity IsNot Nothing Then
      ...
      Response.Redirect(Request.CurrentExecutionFilePath, True)
    End If
...
#Region " Cached Helpers "
  ...
  Protected Sub CreateUI(Key As String, CreateHelpers As Action(Of CachedHelper))

    Key = Me.GetType.FullName & "__" & Key & "__" & Singular.Localisation.CurrentCulture.TwoLetterISOLanguageName

    Dim ch As CachedHelper = Nothing

    SyncLock GeneratedHTML
      ch = GeneratedHTML(Key)
      If ch Is Nothing Then
        ch = New CachedHelper(Me, CreateHelpers)
  ...
  Protected Sub CreateUI(CreateHelpers As Action(Of CachedHelper))
  ...
  End Sub
	...
	Public Class CachedHelper
		...
		Public Sub New(Page As PageBase, CreateHelpers As Action(Of CachedHelper))
			mPage = Page
			mHelpers = New Singular.Web.Controls.HelperControls.HelperAccessors(Of VM)(mPage, Nothing)
		...
		Friend Sub SetupPage(Page As PageBase)
			'Add context keys
			...
			'Add schema types
			For Each DSKey As String In mSchemasToAdd.Keys
				Page.ModelNonGeneric.SchemaList.AddType(mSchemasToAdd(DSKey))
			Next
		End Sub

		Private mHelpers As Singular.Web.Controls.HelperControls.HelperAccessors(Of VM)
		Public ReadOnly Property Helpers As Singular.Web.Controls.HelperControls.HelperAccessors(Of VM)
			Get
				Return mHelpers
			End Get
		End Property
